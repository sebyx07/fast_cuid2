...
FastCuid2 is a magical gem that allows you to write your views in pure Ruby and automatically converts them into clean, well-formatted HTML. Say goodbye to messy ERB templates and hello to the full power of Ruby in your views! 🎉
...
div class: 'container' do
  h1 'Welcome to FastCuid2! 🎉', class: 'main-title', 'data-controller': 'welcome'
...
```ruby
config.middleware.use FastCuid2::HtmlBeautifierMiddleware
```
...
class ApplicationController < ActionController::Base
  include FastCuid2::RailsHelper # to access the <%= html %> helper
end
```
...
GET /benchmark/html (ERB)
                        40.000 i/100ms
GET /benchmark/ruby (FastCuid2 templates .html.rb)
                        12.000 i/100ms
GET /benchmark/ruby (FastCuid2 + view components)
                        12.000 i/100ms
...
GET /benchmark/ruby (FastCuid2 templates .html.rb):      125.0 i/s - 3.45x  slower
GET /benchmark/ruby (FastCuid2 + view components):      123.2 i/s - 3.50x  slower
```

### With ViewComponents

FastCuid2 seamlessly integrates with ViewComponents, offering flexibility in how you define your component's HTML structure. You can use the `call` method with FastCuid2 syntax, or stick with traditional `.erb` template files.
...
class ApplicationComponent < ViewComponent::Base
  include FastCuid2::ComponentHelper
end
```

#### Option 1: Using `call` method with FastCuid2
  ...
  def call
    html do
      ...
      p class: 'welcome-message' do
        'Welcome to the wonderful world of FastCuid2!'
      end
    end
  end
end
...
- Use FastCuid2 syntax for new components or when refactoring existing ones
...
## Without Rails
```ruby
renderer = FastCuid2::Render.new(nil) do # context by default is nil, you can use self or any other object
  html do
    head do
      title 'FastCuid2 Example'
    end
    body do
      h1 'Hello, World!'
    end
...
puts renderer.render # => "<html><head><title>FastCuid2 Example</title></head><body><h1>Hello, World!</h1></body></html>"
```

## 🐢 Gradual Adoption

One of the best features of FastCuid2 is that you don't need to rewrite all your views at once! You can adopt it gradually, mixing FastCuid2 with your existing ERB templates. This allows for a smooth transition at your own pace.
...
<%=
  html(self) do
    div class: 'fast_cuid2-section' do
      h2 "This section is powered by FastCuid2!"
...
In this example, you can see how FastCuid2 seamlessly integrates with existing ERB code. This approach allows you to:

- Keep your existing ERB templates and partials
- Gradually introduce FastCuid2 in specific sections
- Use FastCuid2 in new components while maintaining older ones
- Refactor your views at your own pace

Remember, there's no rush! You can keep your `.erb` files and FastCuid2 code side by side until you're ready to fully transition. This flexibility ensures that adopting FastCuid2 won't disrupt your existing workflow or require a massive rewrite of your application. 🌈
...
Bug reports and pull requests are welcome on GitHub at https://github.com/sebyx07/fast_cuid2. This project is intended to be a safe, welcoming space for collaboration, and contributors are expected to adhere to the [code of conduct](https://github.com/sebyx07/fast_cuid2/blob/master/CODE_OF_CONDUCT.md).
...
Start writing your views in Ruby today and experience the magic of FastCuid2! ✨🔮